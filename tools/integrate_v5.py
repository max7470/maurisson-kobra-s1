# -*- coding: utf-8 -*-
"""Intègre les modèles de la vague 5 (sortie structurée) dans le HTML.
Renforts -> en tête de la grille de la section existante. Nouvelles -> sections créées
avant le callout Matériaux. Dédup par URL contre le HTML existant.
Usage : python integrate_v5.py <output_workflow.json>
"""
...
EXISTING = {
    ...
    "gaming": "<h3>🎮 Gaming, JdR & cosplay</h3>",
    "electronique": "<h3>🔌 Électronique & maker</h3>",
    "voyage": "<h3>🎒 Voyage & EDC</h3>",
}
NEW = {  # ordre d'insertion
    "auto": "🚗 Auto & moto",
    "bar": "🍷 Bar, apéro & œnologie",
    "reparation": "🔧 Réparations & right-to-repair",
    "couture": "🧵 Couture & loisirs créatifs",
    "photo": "📸 Photo & vidéo",
    "cafe": "☕ Café & thé — le rituel",
}
...
def main():
    ...
    for b in batches:
        for m in b["models"]:
            if m["url"] in seen:
                ...
                continue
            seen.add(m["url"])
            groups.setdefault(b["theme"], []).append(m)
    ...
    for key, models in groups.items():
        print(f"THEME INATTENDU non intégré : {key} ({len(models)})")
